...
class Firebase {
  ...
  Future<String> createIncomingInvoice(IncomingInvoice invoice) async {
    try {
      ...
      for (var detail in invoice.details) {
        ...
        await createIncomingInvoiceDetail(detail);
        
        // Cập nhật stock của sản phẩm
        await updateProductStock(detail.productID, detail.quantity);
      }
  ...
  Future<void> updateIncomingInvoice(IncomingInvoice invoice) async {
    try {
      // Lấy hóa đơn cũ để so sánh
      final oldInvoice = await getIncomingInvoiceWithDetails(invoice.incomingInvoiceID!);
      ...
      for (var detail in invoice.details) {
        if (detail.incomingInvoiceDetailID != null) {
          ...
          await updateIncomingInvoiceDetail(detail, oldInvoice);
        } else {
          // Thêm chi tiết mới
          detail.incomingInvoiceID = invoice.incomingInvoiceID!;
          await createIncomingInvoiceDetail(detail);
          await updateProductStock(detail.productID, detail.quantity);
        }
      }
      ...
      final oldDetailIds = oldInvoice.details.map((d) => d.incomingInvoiceDetailID).toSet();
      final newDetailIds = invoice.details.map((d) => d.incomingInvoiceDetailID).toSet();
      final detailsToDelete = oldDetailIds.difference(newDetailIds);

      for (var detailId in detailsToDelete) {
        if (detailId != null) {
          await deleteIncomingInvoiceDetail(detailId);
        }
      }
    } catch (e) {
  ...
  // Cập nhật chi tiết hóa đơn nhập hàng
  Future<void> updateIncomingInvoiceDetail(IncomingInvoiceDetail detail, IncomingInvoice oldInvoice) async {
    try {
      // Tìm chi tiết cũ để so sánh số lượng
      final oldDetail = oldInvoice.details.firstWhere(
        (d) => d.incomingInvoiceDetailID == detail.incomingInvoiceDetailID
      );
      ...
      // Cập nhật stock nếu số lượng thay đổi
      final stockChange = detail.quantity - oldDetail.quantity;
      if (stockChange != 0) {
        await updateProductStock(detail.productID, stockChange);
      }
    } catch (e) {
  ...
  Future<void> deleteIncomingInvoiceDetail(String detailId) async {
    try {
      final detailDoc = await FirebaseFirestore.instance
          .collection('incoming_invoice_details')
          .doc(detailId)
      ...
      print('Error deleting incoming invoice detail: $e');
      rethrow;
    }
  }

  // Lấy hóa đơn nhập hàng với chi tiết
  Future<IncomingInvoice> getIncomingInvoiceWithDetails(String invoiceID) async {
    try {
      ...
      final invoice = IncomingInvoice.fromMap(invoiceID, invoiceDoc.data()!);
      ...
      invoice.details = detailsSnapshot.docs.map((doc) {
        return IncomingInvoiceDetail.fromMap(doc.id, doc.data());
      }).toList();
